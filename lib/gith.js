...
var filterSettings = function( settings, payload ) {
  ...
  [ 'repo', 'branch', 'file', 'tag', 'event' ].forEach( function( thing ) {
...
// Guess event type from payload, this allows user to filter
var guessEvent = function( payload ) {
  if (payload.issue && payload.comment)                     return 'issue_comment';
  else if (payload.issue)                                   return 'issue';
  else if (payload.pull_request)                            return 'pull_request';
  else if (payload.pages)                                   return 'gollum';
  else if (typeof payload.download != 'undefined')          return 'download';
  else if (payload.forkee)                                  return 'fork';
  else if (payload.head && payload.before && payload.after) return 'fork_apply';
  else if (payload.member)                                  return 'member';
  else if (payload.commits)                                 return 'push';
  else if (payload.comment)                                 return 'commit_comment';
  else if (payload.action)                                  return 'status';
  else if (Object.keys(payload) === 2)                      return 'public';
  else                                                      return 'unknown';
};
